...
fn main() {
    ...
    for case in cases {
        let n = case.len();
        let a = case;
        let mut a = a.into_iter().enumerate().map(|(i, ai)| (i as u32, ai)).collect::<Vec<_>>();
        a.sort_unstable_by_key(|&(_, w)| w);
        a.iter_mut()
            .enumerate()
            .for_each(|(i, (_, w))| *w = i as _);
        a.sort_unstable_by_key(|&(i, _)| i);
        // println!("{:?}", bin2);
        let mut lis_last_min = Vec::with_capacity(n);
        let mut dp1 = SegTree::from_iter(iter::repeat(MaxQuery(0u32)).take(n));
        for &(_, ai) in &a {
            let v1 = dp1.query(..ai as usize).0 + 1;
            dp1.update(ai as usize, &MaxQuery(v1));
        }
        for &(_, ai) in &a {
            let i = lis_last_min.partition_point(|&x| x < ai);
            if i == lis_last_min.len() {
                lis_last_min.push(ai);
            } else {
                lis_last_min[i] = ai;
            }
        }
